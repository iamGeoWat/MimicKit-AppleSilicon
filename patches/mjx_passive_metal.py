...
def _spring_damper(m: Model, d: Data) -> jax.Array:
  ...
  # [openksp/metal] ntendon is STATIC, so skip the whole block when there are no tendons:
  # the zero-width slice this emits crashes jax-metal's shape inference
  # ('mps.strided_slice' inferred 1x1 vs 1x0). Skipping is also strictly less work on any
  # backend -- the term is identically zero when ntendon == 0.
  if m.ntendon:
