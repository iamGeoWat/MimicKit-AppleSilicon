...
def solve(m: Model, d: Data) -> Data:
  ...
  if m.opt.iterations == 1:
    ...
    # [openksp/metal] jax.lax.while_loop with this large _Context carry trips jax-metal's
    # parameter marshalling (SEGFAULT). _while_loop_scan is MJX's OWN bounded-while helper,
    # already used for the line search below -- same semantics, fixed trip count.
    import os as _os
    if _os.environ.get("MJX_SCAN_SOLVER", "0") == "1":
