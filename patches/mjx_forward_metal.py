...
def fwd_velocity(m: Model, d: Data) -> Data:
  ...
  # [openksp/metal] ten_J is (0, nv) when ntendon == 0, so this matmul is an EMPTY matmul
  # that jax-metal mis-marshals. ntendon is static, so guard it.
  _upd = {'actuator_velocity': d.actuator_moment @ d.qvel}
  if m.ntendon:
    _upd['ten_velocity'] = d.ten_J @ d.qvel
  d = d.replace(**_upd)
